 ...
 * paste/beforeinput events are ignored by Slate in Firefox).
 * ================================================================== */
(function () {
    'use strict';
    ...
    /* ==================================================================
     * Anonymous chat (runs FIRST, synchronously, at document_start).
     * Twitch's chat websocket authenticates with "PASS oauth:…" +
     * "NICK <login>"; rewriting those to the anonymous justinfan login
     * keeps you out of the viewer list (chat becomes read-only).
     * Settings load is async, so the toggle is mirrored into page
     * localStorage (key below) and read back synchronously here — the
     * shim must be installed before Twitch's own scripts run.
     * Firefox-only mechanics: the page's WebSocket.prototype.send is
     * patched across the Xray boundary with exportFunction.
     * ================================================================== */
    const ANON_LS_KEY = 'ytbtw-anon';
    function installAnonShim() {
        try {
            if (IS_CLIPS_HOST || IS_MOBILE) return;
            if (localStorage.getItem(ANON_LS_KEY) !== '1') return;
            if (typeof exportFunction !== 'function' || !window.wrappedJSObject) return;
            const pw = window.wrappedJSObject;
            const proto = pw.WebSocket.prototype;
        ...
        } catch (e) { /* Xray plumbing unavailable; feature silently off */ }
    }
    installAnonShim();
    ...
    // in Firefox, so the editor object itself is the reliable path.
    function chatEditor() {
        try {
            const input = document.querySelector('div[data-a-target="chat-input"]');
            if (!input) return null;
            const raw = input.wrappedJSObject || input;   // see Xray note above
            const fiberKey = Object.keys(raw).find(k => k.startsWith('__reactFiber$'));
            if (!fiberKey) return null;
            let node = raw[fiberKey];
    ...
    function chatSend(text) {
        if (!chatInsert(text)) return false;
        setTimeout(() => {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        const editor = chatEditor();
        if (!editor || typeof editor.deleteBackward !== 'function') { tabState = null; return; }
        e.preventDefault();
        e.stopPropagation();
        try {
            for (let i = 0; i < st.lastLen; i++) editor.deleteBackward('character');
            const name = st.cands[st.idx];
            editor.insertText(name);
            st.lastLen = name.length;
            tabState = st;
        } catch (err) { tabState = null; }
    }, true);
    ...
    function getChannelId() {
        try {
            const input = document.querySelector('div[data-a-target="chat-input"]');
            if (!input) return null;
            const raw = input.wrappedJSObject || input;
            const fk = Object.keys(raw).find(k => k.startsWith('__reactFiber$'));
            if (!fk) return null;
            let node = raw[fk];
            ...
            for (const scope of ['channel', 'global']) {
                ...
                for (const [name, e] of items) {
                    ...
                    b.addEventListener('click', (ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
                        if (!chatInsert(name + ' ')) toast('Couldn’t reach the chat input', name);
                    });
                    grid.appendChild(b);
                }
                if (shown > 400) break;
    ...
    function streamStartFromApollo(login) {
        try {
            ...
            const raw = rootEl && (rootEl.wrappedJSObject || rootEl);
            if (!raw) return 0;
            const fk = Object.keys(raw).find(k =>
