...
(function () {
    ...
    // Firefox for Android has no context menus; everything else works there.
    ...
    api.runtime.onInstalled.addListener((details) => {
        ...
        // One-time repair when updating from the 4.0.0/4.1.0 test builds:
        // Firefox's stale startup cache could serve an old settings page
        // right after the in-place update, and a save from such a page wrote
        // explicit `false` for toggles it didn't know about. Drop exactly
        // those keys so the shipped defaults re-apply; twAnonChat is kept
        // because turning it on is a deliberate choice.
        if (details && details.reason === 'update' && /^4\.(0\.0|1\.0|1\.1)$/.test(details.previousVersion || '')) {
            api.storage.local.get('data').then(r => {
                const d = r.data;
                if (!d || !d.settings) return;
                ['twCinemaButton', 'twEmotes', 'twAutoClaimDrops', 'twAutoClaimMoments',
                 'twClipDownload', 'ytCinemaButton'].forEach(k => { delete d.settings[k]; });
                return api.storage.local.set({ data: d });
            }).catch(() => {});
        }
    });
    ...
    api.runtime.onMessage.addListener((msg, sender) => {
        ...
        if (msg.action === 'ytb-sb-segments') {
            return sbSegments(String(msg.videoId || ''), Array.isArray(msg.categories) && msg.categories.length ? msg.categories : ['sponsor']);
        }
        if (msg.action === 'ytb-de-branding') {
            return deBranding(String(msg.videoId || ''));
        }
